...
package com.mynt.banking.currency_cloud.pay.payers;
...
import com.mynt.banking.currency_cloud.pay.payers.requests.*;
...
public class CurrencyCloudPayersService {
    private final CurrencyCloudClient currencyCloudClient;
    private final CurrencyCloudAuthenticator currencyCloudAuthenticator;
    
    public ResponseEntity<JsonNode> getPayer(
            String id,
			CurrencyCloudGetPayerRequest request
    ) {
        String uri = UriBuilderUtil.buildUriWithQueryParams("/v2/payers/" + id + "", request);
